...
验收按 Step 0–11 严格顺序进行，未通过当前步骤不得执行后续步骤。

| 步骤 | 状态 | 证据 |
|---|---|---|
| 0 路径与工程约定 | 通过 | 路径检查成功；`tests/test_paths.py` 5 项通过；原始/派生数据未被 Git 跟踪 |
| 1 全量真实数据 | 通过 | 本地与服务器 109,320 文件逐路径、大小、SHA-256 全一致；9 张关键表可读 |
| 2 黄金测试数据 | 通过 | 5 只人工股票、34 个交易日；5 项契约测试通过，重建一致 |
| 3 数据画像 | 通过 | 69 张真实业务表完整扫描；9 核心表字段确认；19 项累计测试通过 |
| 4 交易日历 | 通过 | 真实 SSE 2,012 个交易日、主键唯一；18 项日历测试通过 |
| 5 复权价格与收益率 | 通过 | 9,415,148 行；15 项测试通过；全量复权与断档错误 0 |
| 6 每日证券状态 | 可用部分通过；缺源项挂起 | 11,872,812 行、12 项测试通过；退市前 288,376 行正常上市事实保留 |
| 7 每日买卖可行性 | 可用部分通过；缺源项挂起 | 11,872,812 行；16 项测试；全量方向/停牌/BJ未知约束错误 0 |
...
| 10 研究样本空间 | 可用部分通过；ST 样本挂起 | 10 项测试；all_a 9,414,104 成员证券日；退市前 288,376 行保留 |
| 11 统一质量与阶段验收 | 实施中 | Step 10 全量验收后开始 |
